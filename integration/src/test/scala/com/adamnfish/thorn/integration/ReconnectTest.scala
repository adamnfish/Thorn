...
class ReconnectTest extends AnyFreeSpec with AttemptValues with OptionValues with ThornIntegration {
  ...
  "for a valid request" - {
    "is successful" in {
      withTestContext { (context, _) =>
        ...
        Fixtures.reconnect(creatorWelcome, context(PlayerAddress("new address"))).isSuccessfulAttempt()
      }
    }
    ...
    "returns a game status to the player" in {
      withTestContext { (context, _) =>
        ...
        val gameStatus = Fixtures.reconnect(creatorWelcome, context(PlayerAddress("new address"))).value().response.value
    ...
    "doesn't send any other messages" in {
      withTestContext { (context, _) =>
        ...
        val messages = Fixtures.reconnect(creatorWelcome, context(PlayerAddress("new address"))).value().messages
